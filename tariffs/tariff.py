                        ...
                        if charge.time and charge.season:
                                ...
                                for period in charge.time.periods:
                                    if period.from_weekday <= dt.dayofweek <= period.to_weekday and datetime.time(
                                        ...
                                        charge_array, block_accum_dict = self.calc_charge(
                                            str(charge.code) + self.service + charge_type + charge.season.name + charge.time.name, row,
                                            charge, charge_array, block_accum_dict)
                                        found = True
                                        break
                            if not found:
                                charge_array[str(charge.code) + self.service + charge_type + charge.season.name + charge.time.name].append(0.0)
                        elif charge.season and not charge.time:
                            if datetime.date(year=dt.year, month=charge.season.from_month, day=charge.season.from_day)\
                                    <= dt.date() <= datetime.date(year=dt.year, month=charge.season.to_month,
                                                                 day=charge.season.to_day):
                                charge_array, block_accum_dict = self.calc_charge(
                                    str(charge.code) + self.service + charge_type + charge.season.name, row, charge, charge_array, block_accum_dict)
                            else:
                                charge_array[str(charge.code) + self.service + charge_type + charge.season.name].append(0.0)
                            ...
                            for period in charge.time.periods:
                                if period.from_weekday <= dt.dayofweek <= period.to_weekday and datetime.time(
                                        ...
                                        hour=period.to_hour, minute=period.to_minute):
                                    charge_array, block_accum_dict = self.calc_charge(
                                        str(charge.code) + self.service + charge_type + charge.time.name, row, charge, charge_array, block_accum_dict)
                                    found = True
                            if not found:
                                charge_array[str(charge.code) + self.service + charge_type + charge.time.name].append(0.0)
                        elif charge.rate_schedule:
                            for schedule_item in charge.rate_schedule:
                                if dt.to_pydatetime() < schedule_item.datetime:
                                    charge_array[str(charge.code) + self.service + charge_type + 'scheduled'].append(schedule_item.rate * float(row[charge.meter]))
                                    break
                        else:
                            charge_array, block_accum_dict = self.calc_charge(
                                str(charge.code) + self.service + charge_type, row, charge, charge_array, block_accum_dict)
